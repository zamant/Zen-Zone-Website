...
class Calendar {   
    ...
    */
	private function _showDay($cellNumber,$attributes=false){ 
		...
		return '<li id="li-'.$this->currentDate.'" class="'.($cellNumber%7==1?' start ':($cellNumber%7==0?' end ':' ')).
				($cellContent==null?'inactive':'').'">'.$cellContent.'</li>';
	}
    ...
    */
	private function _createNavi(){
		...
		return 		
			'<div id="navi">'.
			'<div id="prev"><a class="button" href="'.$this->naviHref.sprintf("%02d", $preMonth).'/'.$preYear.'"><span>Prev</span></a></div>'.
				date('Y M',strtotime($this->currentYear.'-'.$this->currentMonth.'-1')).
			'<div id="next"><a class="button" href="'.$this->naviHref.sprintf("%02d", $nextMonth).'/'.$nextYear.'"><span>Next</span></a></div>'.
			'</div>'.
    ...
    */
	private function _createLabels(){
		//v.1.1.1 01 April: bug fix on labels
		if($this->sundayFirst){
    ...
    */	
	private function _link(&$content,$config=false){
		...
		return $content='<a id="a-'.$this->currentDate.'" class="common" href="'.
				...
				date('Y',strtotime($this->currentDate)).'/'.date('m',strtotime($this->currentDate)).'/'.date('d',strtotime($this->currentDate)).'">'.$content.'</a>';	
	}
